...
def normalize_maxmin(pose_2d):
    ...
    min_xy = np.min(pose_2d[:, :, [0, 1]], axis=1, keepdims=True)
    max_xy = np.max(pose_2d[:, :, [0, 1]], axis=1, keepdims=True)
    diff_xy = max_xy - min_xy
    max_range = np.max(diff_xy, axis=2, keepdims=True)
    pose_2d[:, :, [0, 1]] = (pose_2d[:, :, [0, 1]] - min_xy) / max_range
    return pose_2d
