...
def convert_mpi_to_pos_rot(poses: np.ndarray) -> (np.ndarray, np.ndarray):
    poses = poses.reshape((poses.shape[0], poses.shape[1] // 3, 3))  # n x 28 x 3
    print(poses.shape[0])
    ...
    head_top = poses[:, 7]  # 1 x n x 3
    # face = poses[:, 14] does not have a face point
    neck = poses[:, 5]
    head_center = (head_top + neck) / 2
    left_hand_center = poses[:, 12]
    right_hand_center = poses[:, 17]
    left_hip = poses[:, 18]
    right_hip = poses[:, 23]
    hip_center = (left_hip + right_hip) / 2
    left_shoulder = poses[:, 9]
    right_shoulder = poses[:, 14]
    ...
    head_quat = np.asarray([np.quaternion(1, 0, 0, 0)] * head_center.shape[0])
    left_hand_quat = np.asarray([np.quaternion(1, 0, 0, 0)] * head_center.shape[0])
    right_hand_quat = np.asarray([np.quaternion(1, 0, 0, 0)] * head_center.shape[0])
